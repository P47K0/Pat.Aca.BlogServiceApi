...
//
// GET /search reuses this same function under its own RateLimitConfig
// (see index.ts) rather than duplicating the fixed-window logic -- a
// different keyPrefix keeps the two endpoints' counters independent, so a
// visitor's search activity never eats into their /ask budget or vice versa.
...
const ASK_RATE_LIMIT: RateLimitConfig = {
  keyPrefix: 'ratelimit',
...
function rateLimitKey(config: RateLimitConfig, clientIp: string): string {
  return `${config.keyPrefix}:${clientIp}`;
}

/** Increments clientIp's request count for the current fixed window and
 * reports whether this request is still within config.limitPerWindow.
 * Defaults to ASK_RATE_LIMIT when no config is given, preserving every
 * existing POST /ask call site's behavior unchanged. Read-then-write
 * against KV, not atomic -- a race between two near-simultaneous requests
 * from the same IP could let one extra slip through, the same accepted
 * trade-off as semantic-cache.ts's addCacheEntry, and fine at this Worker's
 * expected traffic (a personal-site chat widget, not a public API built to
 * withstand a coordinated flood).
 *
...
export async function checkRateLimit(
  kv: KVNamespace,
  clientIp: string,
  config: RateLimitConfig = ASK_RATE_LIMIT,
): Promise<boolean> {
  const key = rateLimitKey(config, clientIp);
  const now = Date.now();
  const stored = await kv.get<RateLimitState>(key, 'json');

  const withinCurrentWindow = stored !== null && now - stored.windowStartedAt < config.windowSeconds * 1000;
  const state: RateLimitState = withinCurrentWindow
    ? { count: stored!.count + 1, windowStartedAt: stored!.windowStartedAt }
    : { count: 1, windowStartedAt: now };

  await kv.put(key, JSON.stringify(state), { expirationTtl: config.windowSeconds * 2 });

  return state.count <= config.limitPerWindow;
}
